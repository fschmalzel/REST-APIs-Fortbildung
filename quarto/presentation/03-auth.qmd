...
## Übung 4 {.smaller}
Basis-URL: `https://rest.fschmalzel.de/api/v1/messages` **(Ohne `public/`)**

Message Board ansehen: <https://rest.fschmalzel.de/board>
...
2. Registriert euch:
```sh
curl https://rest.fschmalzel.de/api/v1/auth/register \
  -X POST \
  --json '{"username": "EUER_NAME", "password": "abc123"}'
```
3. Loggt euch ein — was bekommt ihr zurück?
```sh
curl https://rest.fschmalzel.de/api/v1/auth/login \
  -X POST \
  --json '{"username": "EUER_NAME", "password": "abc123"}'
```
